@php
/**
 * YCX Insights Articles Dictionary
 */
$articles = [
    'why-every-professional-has-story-worth-sharing' => [
    ...
    'power-of-meaningful-connections-career-growth' => [
        'category' => 'Networking',
    ...
    'learning-beyond-classroom-why-real-experiences-matter' => [
        'category' => 'Learning',
    ...
    'art-of-pitching-telling-story-that-secures-partners' => [
        'category' => 'Storytelling',
    ...
    'building-strong-personal-brand-through-authentic-experiences' => [
    ...
    'from-conversations-to-collaborations-creating-opportunities-through-networking' => [
    ...
    'lessons-from-entrepreneurs-turning-challenges-into-opportunities' => [
    ...
    'future-of-communities-connecting-people-through-knowledge-and-experiences' => [
        'category' => 'Collaboration',
...
// Fallback to the first article if slug is not found or empty
$slug = $slug ?? 'why-every-professional-has-story-worth-sharing';
$article = $articles[$slug] ?? $articles['why-every-professional-has-story-worth-sharing'];

// Set page SEO dynamically
$seo = [
    'title'       => $article['title'] . ' — Young Chanakya X',
    'description' => strip_tags(Str::limit($article['content'], 160)),
    'keywords'    => 'ycx insights, ' . $article['category'] . ', young chanakya x, professional insights',
    'image'       => $article['image'],
    'type'        => 'article',
];
@endphp
...
<div class="blog-details-body">
    ...
    <!-- HERO SECTION -->
    <div class="premium-hero-container d-flex align-items-center position-relative" style="min-height: 75vh; background-image: linear-gradient(180deg, rgba(12, 58, 48, 0.65) 0%, #090d16 100%), url('{{ $article['image'] }}'); background-size: cover; background-position: center; background-attachment: fixed;">
            ...
            <div class="row">
                <div class="col-xl-9 col-lg-11">
                    <ul class="style-none d-flex align-items-center gap-2 p-0 mb-4 text-uppercase tracking-wider" style="font-size: 0.75rem; font-weight: 700; letter-spacing: 2px; list-style: none;">
                        ...
                        <li class="text-white opacity-40">/</li>
                        <li><a href="#" class="text-white opacity-60 text-decoration-none hover-opacity-100">{{ $article['category'] }}</a></li>
                        <li class="text-white opacity-40">/</li>
                        <li class="text-white" style="color: #ffd2b1 !important;">{{ Str::limit($article['title'], 35) }}</li>
                    </ul>
                    <h1 class="text-glow-headline mb-4" style="font-size: clamp(32px, 4.5vw, 56px);">{{ $article['title'] }}</h1>
                    <div class="d-flex align-items-center gap-3 text-white">
                        <img src="{{ $article['author']['avatar'] }}" alt="{{ $article['author']['name'] }}" style="width: 44px; height: 44px; border-radius: 50%; object-fit: cover;">
                        <div>
                            <div style="font-weight: 700; font-size: 14px;">{{ $article['author']['name'] }}</div>
                            <div style="font-size: 11px; opacity: 0.75;" class="text-uppercase tracking-wider">{{ $article['date'] }} • {{ $article['read_time'] }}</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="article-layout">
                ...
                <div class="article-sidebar">
                    ...
                    <div class="sidebar-box">
                        ...
                        @foreach(Collect($articles)->except($slug)->take(3) as $k => $item)
                        <div style="margin-bottom: 20px; border-bottom: 1px solid rgba(12, 58, 48, 0.04); padding-bottom: 16px;">
                            ...
                            <a href="{{ route('insights.detail', $k) }}" class="recent-link" style="margin-top: 4px;">{{ $item['title'] }}</a>
                        </div>
                        @endforeach
                    </div>
