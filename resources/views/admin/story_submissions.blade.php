...
@section('title', 'Story Talks — ConnectX Admin')

@section('content')
<main class="main">
  <div class="topbar">
    <div>
      <h1 id="pageTitle">Story Talks Submissions</h1>
      <p id="pageSubtitle">Review and manage community story submissions and speaker proposals.</p>
    </div>
    <div class="search-box">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
      <input type="text" id="searchInput" placeholder="Search talk titles or applicant names…">
    </div>
  </div>
  ...
  <div class="filter-bar" id="filterBar" style="display:flex; gap:12px; align-items:center;">
    <select id="langFilter" style="font-family:inherit; font-size:13.5px; border:1px solid var(--border); border-radius:8px; padding:10px 14px; background:var(--white); color:var(--ink); outline:none;">
  ...
  <div class="table-card" id="tableCard">
    ...
    <div class="empty-state" id="emptyState" style="display:none;">
      <h3>No story talk submissions match these filters</h3>
...
<div class="drawer" id="drawer">
  <div class="drawer-head">
    <div class="dh-top">
      <div>
        <h3 id="dName">Talk Proposal Details</h3>
        <p id="dSub"></p>
      </div>
...
let currentPage = 1;
let activeType = 'all';
let DATA = { story_submissions: @json($storySubmissions) };

const SECTION_CONFIG = {
  story_submissions: {
    title: "Story Talks",
    subtitle: "Review and manage community story submissions and speaker proposals.",
...
function getFiltered(){
  ...
  return DATA.story_submissions.filter(d => {
    if(activeType !== 'all' && d.speaking_language !== activeType) return false;
    if(status !== 'all' && d.status !== status) return false;
    if(search) {
      const matchName = d.name ? d.name.toLowerCase().includes(search) : false;
      const matchTitle = d.talk_title ? d.talk_title.toLowerCase().includes(search) : false;
...
function renderStats(){
  ...
  const data = DATA.story_submissions;
  const STAT_ICONS = [
...
function renderLangFilter(){
  ...
  const values = [...new Set(DATA.story_submissions.map(d => d.speaking_language))].filter(Boolean);
...
function renderTable(){
  ...
  tbody.querySelectorAll('.status-select').forEach(sel => {
    sel.addEventListener('change', async function(e){
      ...
      const item = DATA.story_submissions.find(x => x.id === idVal);
      if(item){
        item.status = this.value;
        this.className = `status-select ${statusClass(this.value)}`;
        renderStats();
        await fetch(`/admin/api/story-submissions/${idVal}`, {
          method: 'POST',
  ...
  return `
    <div class="dsection">
      ...
      <div class="dgrid">
        ...
        <div class="dfield full"><span class="fl">Social Profile</span><a href="${d.social_url}" target="_blank" class="fv cell-link">${d.social_url}</a></div>
        <div class="dfield full"><span class="fl">Proposed Talk Title</span><span class="fv" style="font-weight:700;">${d.talk_title}</span></div>
        <div class="dfield full"><span class="fl">Submitted On</span><span class="fv">${fmtDate(d.submitted)}</span></div>
      </div>
    </div>
    <div class="dsection">
      <h4>Talk Summary / Overview</h4>
...
function openDrawer(id){
  const d = DATA.story_submissions.find(x => x.id === id);
  if(!d) return;

  document.getElementById('dName').textContent = d.name;
  document.getElementById('dSub').textContent = d.email;
  document.getElementById('dBadges').innerHTML = `<span class="badge badge-platinum">Talk Proposal</span>`;
  document.getElementById('drawerBody').innerHTML = storyBody(d);
  ...
  if(saveBtn){
    saveBtn.onclick = async () => {
      ...
      renderStats();
      renderTable();
      await fetch(`/admin/api/story-submissions/${id}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
...
document.getElementById('langFilter').addEventListener('change', (e) => { activeType = e.target.value; currentPage = 1; renderTable(); });
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    const res = await fetch("{{ route('admin.api.story-submissions') }}");
    if(res.ok) { DATA.story_submissions = await res.json(); }
  } catch(e){}
  renderStats();
  renderLangFilter();
  renderStatusFilter();
