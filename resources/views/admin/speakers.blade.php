...
@section('title', 'Speakers — ConnectX Admin')

@section('content')
<main class="main">
  <div class="topbar">
    <div>
      <h1 id="pageTitle">Speakers</h1>
      <p id="pageSubtitle">Manage keynote speakers, panelist registrations, and session submissions.</p>
    </div>
    <div class="search-box">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
      <input type="text" id="searchInput" placeholder="Search speakers…">
    </div>
  </div>
  ...
  <div class="table-card" id="tableCard">
    ...
    <div class="empty-state" id="emptyState" style="display:none;">
      <h3>No speaker entries match these filters</h3>
...
<div class="drawer" id="drawer">
  <div class="drawer-head">
    <div class="dh-top">
      <div>
        <h3 id="dName">Speaker Details</h3>
        <p id="dSub"></p>
      </div>
...
let currentPage = 1;
let activeType = 'all';
let DATA = { speakers: @json($speakers) };

const SECTION_CONFIG = {
  speakers: {
    title: "Speakers",
    subtitle: "Manage keynote speakers, panelist registrations, and session submissions.",
    statusOptions: ["pending", "confirmed", "declined"],
    statusLabels: { pending: "Pending Review", confirmed: "Confirmed", declined: "Declined" },
    typeField: "talk_category",
    typeValues: [],
    columns: [
      { key:"person", label:"Speaker & Email" },
      { key:"company", label:"Organization & Title" },
      { key:"phone", label:"Phone" },
      { key:"talk_category", label:"Talk Category" },
      { key:"submitted", label:"Submitted Date" },
      { key:"status", label:"Status" },
      { key:"action", label:"Actions" }
    ],
    stats: [
      { label:"Total Speakers", fn: d => d.length },
...
function getFiltered(){
  ...
  return DATA.speakers.filter(d => {
    if(activeType !== 'all' && d.talk_category !== activeType) return false;
...
function renderTable(){
  ...
  tbody.querySelectorAll('.status-select').forEach(sel => {
    sel.addEventListener('change', async function(e){
      ...
      const item = DATA.speakers.find(x => x.id === idVal);
      if(item){
        item.status = this.value;
        this.className = `status-select ${statusClass(this.value)}`;
        renderStats();
        await fetch(`/admin/api/speakers/${idVal}`, {
          method: 'POST',
  ...
  return `
    <div class="dsection">
      ...
      <div class="dgrid">
        ...
        <div class="dfield"><span class="fl">Company</span><span class="fv">${d.company}</span></div>
        <div class="dfield"><span class="fl">Designation</span><span class="fv">${d.designation}</span></div>
        <div class="dfield"><span class="fl">Talk Category</span><span class="fv">${d.talk_category}</span></div>
        <div class="dfield full"><span class="fl">LinkedIn Profile</span><a href="${d.linkedin}" target="_blank" class="fv cell-link">${d.linkedin}</a></div>
...
function openDrawer(id){
  const d = DATA.speakers.find(x => x.id === id);
  if(!d) return;

  document.getElementById('dName').textContent = d.name;
  document.getElementById('dSub').textContent = d.email;
  document.getElementById('dBadges').innerHTML = `<span class="badge badge-platinum">Speaker</span>`;
  document.getElementById('drawerBody').innerHTML = speakerBody(d);
  ...
  if(saveBtn){
    saveBtn.onclick = async () => {
      ...
      renderStats();
      renderTable();
      await fetch(`/admin/api/speakers/${id}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    const res = await fetch("{{ route('admin.api.speakers') }}");
    if(res.ok) { DATA.speakers = await res.json(); }
  } catch(e){}
  renderStats();
  renderStatusFilter();
  renderTable();
