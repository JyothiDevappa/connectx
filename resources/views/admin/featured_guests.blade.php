...
@section('title', 'Guest Features — ConnectX Admin')

@section('content')
<main class="main">
  <div class="topbar">
    <div>
      <h1 id="pageTitle">Guest Features</h1>
      <p id="pageSubtitle">Review and manage guest spotlight applications and feature requests.</p>
    </div>
    <div class="search-box">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
      <input type="text" id="searchInput" placeholder="Search guests by name or company…">
    </div>
  </div>
  ...
  <div class="table-card" id="tableCard">
    ...
    <div class="empty-state" id="emptyState" style="display:none;">
      <h3>No featured guest applications match these filters</h3>
...
<div class="drawer" id="drawer">
  <div class="drawer-head">
    <div class="dh-top">
      <div>
        <h3 id="dName">Featured Guest Details</h3>
        <p id="dSub"></p>
      </div>
...
let currentPage = 1;
let activeType = 'all';
let DATA = { featured_guests: @json($featuredGuests) };

const SECTION_CONFIG = {
  featured_guests: {
    title: "Guest Features",
    subtitle: "Review and manage guest spotlight applications and feature requests.",
    statusOptions: ["pending", "approved", "declined"],
    statusLabels: { pending: "Pending Review", approved: "Approved", declined: "Declined" },
    typeField: "company_name",
    typeValues: [],
    columns: [
...
function getFiltered(){
  ...
  return DATA.featured_guests.filter(d => {
    ...
    if(search) {
      ...
      const matchComp = d.company_name ? d.company_name.toLowerCase().includes(search) : false;
...
function renderStats(){
  ...
  const data = DATA.featured_guests;
  const STAT_ICONS = [
...
function renderCell(col, d){
  switch(col.key){
    ...
    case 'company':
      return `<td><div class="cell-primary">${d.company_name||'—'}</div><div class="cell-secondary">${d.designation||''}</div></td>`;
    case 'submitted':
...
function renderTable(){
  ...
  tbody.querySelectorAll('.status-select').forEach(sel => {
    sel.addEventListener('change', async function(e){
      ...
      const item = DATA.featured_guests.find(x => x.id === idVal);
      if(item){
        item.status = this.value;
        this.className = `status-select ${statusClass(this.value)}`;
        renderStats();
        await fetch(`/admin/api/featured-guests/${idVal}`, {
          method: 'POST',
...
function openDrawer(id){
  const d = DATA.featured_guests.find(x => x.id === id);
  if(!d) return;

  document.getElementById('dName').textContent = d.name;
  document.getElementById('dSub').textContent = d.email;
  document.getElementById('dBadges').innerHTML = `<span class="badge badge-platinum">Featured Guest</span>`;
  document.getElementById('drawerBody').innerHTML = featuredGuestBody(d);
  ...
  if(saveBtn){
    saveBtn.onclick = async () => {
      ...
      renderStats();
      renderTable();
      await fetch(`/admin/api/featured-guests/${id}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    const res = await fetch("{{ route('admin.api.featured-guests') }}");
    if(res.ok) { DATA.featured_guests = await res.json(); }
  } catch(e){}
  renderStats();
  renderStatusFilter();
  renderTable();
