...
@section('title', 'Partners — ConnectX Admin')
...
<main class="main">
  <div class="topbar">
    <div>
      ...
      <p id="pageSubtitle">Manage strategic partnership applications, corporate collaborations, and partner statuses.</p>
    </div>
  ...
  <div class="table-card" id="tableCard">
    ...
    <div class="empty-state" id="emptyState" style="display:none;">
      <h3>No partner entries match these filters</h3>
...
<div class="drawer" id="drawer">
  <div class="drawer-head">
    <div class="dh-top">
      <div>
        <h3 id="dName">Partner Details</h3>
        <p id="dSub"></p>
      </div>
...
let currentPage = 1;
let activeType = 'all';
let DATA = { partners: @json($partners) };

const SECTION_CONFIG = {
  partners: {
    title: "Partners",
    subtitle: "Manage strategic partnership applications, corporate collaborations, and partner statuses.",
    ...
    stats: [
      { label:"Total Partners", fn: d => d.length },
...
function getFiltered(){
  ...
  return DATA.partners.filter(d => {
    if(activeType !== 'all' && d.partnership_type !== activeType) return false;
...
function renderTypeChips(){
  ...
  const values = [...new Set(DATA.partners.map(d => d.partnership_type))].filter(Boolean);
...
function renderTable(){
  ...
  tbody.querySelectorAll('.status-select').forEach(sel => {
    sel.addEventListener('change', async function(e){
      ...
      const item = DATA.partners.find(x => x.id === idVal);
      if(item){
        item.status = this.value;
        this.className = `status-select ${statusClass(this.value)}`;
        renderStats();
        await fetch(`/admin/api/partners/${idVal}`, {
          method: 'POST',
  ...
  return `
    <div class="dsection">
      ...
      <div class="dgrid">
        ...
        <div class="dfield"><span class="fl">Contact Name</span><span class="fv">${d.name}</span></div>
...
function openDrawer(id){
  const d = DATA.partners.find(x => x.id === id);
  if(!d) return;

  document.getElementById('dName').textContent = d.name;
  document.getElementById('dSub').textContent = d.email;
  document.getElementById('dBadges').innerHTML = `<span class="badge ${levelBadgeClass(d.partnership_type)}">${d.partnership_type} Partner</span>`;
  document.getElementById('drawerBody').innerHTML = partnerBody(d);
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    if(res.ok) { DATA.partners = await res.json(); }
  } catch(e){}
  renderStats();
  renderTypeChips();
  renderStatusFilter();
