...
@section('title', 'Internships — ConnectX Admin')

@section('content')
<main class="main">
  <div class="topbar">
    <div>
      <h1 id="pageTitle">Internship Applications</h1>
      <p id="pageSubtitle">Review and process student internship applications.</p>
    </div>
    <div class="search-box">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
      <input type="text" id="searchInput" placeholder="Search applicant name or email…">
    </div>
  </div>
  ...
  <div class="filter-bar" id="filterBar" style="display:flex; gap:12px; align-items:center;">
    <select id="roleFilter" style="font-family:inherit; font-size:13.5px; border:1px solid var(--border); border-radius:8px; padding:10px 14px; background:var(--white); color:var(--ink); outline:none;">
      <option value="all">All Roles Applied</option>
    </select>
  ...
  <div class="table-card" id="tableCard">
    ...
    <div class="empty-state" id="emptyState" style="display:none;">
      <h3>No internship applications match these filters</h3>
...
<div class="drawer" id="drawer">
  <div class="drawer-head">
    <div class="dh-top">
      <div>
        <h3 id="dName">Internship Details</h3>
        <p id="dSub"></p>
      </div>
...
let currentPage = 1;
let activeType = 'all';
let DATA = { internships: @json($internships) };

const SECTION_CONFIG = {
  internships: {
    title: "Internships",
    subtitle: "Review and process student internship applications.",
    statusOptions: ["pending", "hired", "rejected"],
    statusLabels: { pending: "Pending Review", hired: "Hired", rejected: "Rejected" },
    typeField: "job_title",
    typeValues: [],
    columns: [
      { key:"person", label:"Applicant & Email" },
      { key:"job_title", label:"Internship Role" },
...
function getFiltered(){
  ...
  return DATA.internships.filter(d => {
    if(activeType !== 'all' && d.job_title !== activeType) return false;
...
function renderStats(){
  const cfg = SECTION_CONFIG.internships;
  const data = DATA.internships;
  const STAT_ICONS = [
...
function renderRoleFilter(){
  ...
  const values = [...new Set(DATA.internships.map(d => d.job_title))].filter(Boolean);
  sel.innerHTML = `<option value="all">All Roles Applied</option>` +
...
function renderTable(){
  ...
  tbody.querySelectorAll('.status-select').forEach(sel => {
    sel.addEventListener('change', async function(e){
      ...
      const item = DATA.internships.find(x => x.id === idVal);
      if(item){
        item.status = this.value;
        this.className = `status-select ${statusClass(this.value)}`;
        renderStats();
        await fetch(`/admin/api/job-applications/${idVal}`, {
          method: 'POST',
  ...
  return `
    <div class="dsection">
      ...
      <div class="dgrid">
        ...
        <div class="dfield"><span class="fl">Internship Role Applied</span><span class="fv" style="font-weight:700;">${d.job_title}</span></div>
        <div class="dfield full"><span class="fl">Residential Address</span><span class="fv">${d.residential_address||'—'}</span></div>
      </div>
    </div>
    <div class="dsection">
      <h4>Emergency Contact</h4>
      <div class="dgrid">
        <div class="dfield"><span class="fl">Contact Name</span><span class="fv">${d.emergency_name||'—'}</span></div>
        <div class="dfield"><span class="fl">Contact Phone</span><span class="fv">${d.emergency_phone||'—'}</span></div>
      </div>
    </div>
    <div class="dsection">
      <h4>Education & Professional Profiles</h4>
      <div class="dgrid">
        <div class="dfield"><span class="fl">Currently Pursuing Education</span><span class="fv" style="text-transform:capitalize;">${d.pursuing_education||'Yes'}</span></div>
        <div class="dfield"><span class="fl">College / University</span><span class="fv">${d.college||d.institution||'—'}</span></div>
        <div class="dfield"><span class="fl">Course / Degree</span><span class="fv">${d.course||d.degree||'—'}</span></div>
        <div class="dfield"><span class="fl">Semester / Year of Passing</span><span class="fv">${d.semester||d.year_of_passing||'—'}</span></div>
        <div class="dfield full"><span class="fl">LinkedIn Profile</span>${d.linkedin_url?`<a href="${d.linkedin_url}" target="_blank" class="fv cell-link">${d.linkedin_url}</a>`:'<span class="fv">—</span>'}</div>
        <div class="dfield full"><span class="fl">Resume / CV</span>${d.resume_url?`<a href="${d.resume_url}" target="_blank" class="fv cell-link" style="font-weight:700;">View Resume PDF / Document</a>`:'<span class="fv">—</span>'}</div>
      </div>
    </div>
    <div class="dsection">
...
function openDrawer(id){
  const d = DATA.internships.find(x => x.id === id);
  if(!d) return;

  document.getElementById('dName').textContent = d.name;
  document.getElementById('dSub').textContent = d.email;
  document.getElementById('dBadges').innerHTML = `<span class="badge badge-platinum">Internship Applicant</span>`;
  document.getElementById('drawerBody').innerHTML = internshipBody(d);
...
document.getElementById('roleFilter').addEventListener('change', (e) => { activeType = e.target.value; currentPage = 1; renderTable(); });
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    if(res.ok) {
      ...
      DATA.internships = allApps.filter(x => x.category === 'internship');
    }
  } catch(e){}
  renderStats();
  renderRoleFilter();
