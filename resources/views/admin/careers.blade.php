...
@section('title', 'Careers — ConnectX Admin')

@section('content')
<main class="main">
  <div class="topbar">
    <div>
      <h1 id="pageTitle">Careers Applications</h1>
      <p id="pageSubtitle">Manage job candidate applications and review applicant resumes.</p>
    </div>
  ...
  <div class="table-card" id="tableCard">
    ...
    <div class="empty-state" id="emptyState" style="display:none;">
      <h3>No career applications match these filters</h3>
...
<div class="drawer" id="drawer">
  <div class="drawer-head">
    <div class="dh-top">
      <div>
        <h3 id="dName">Application Details</h3>
        <p id="dSub"></p>
      </div>
...
let currentPage = 1;
let activeType = 'all';
let DATA = { careers: @json($careers) };

const SECTION_CONFIG = {
  careers: {
    title: "Careers",
    subtitle: "Manage job candidate applications and review applicant resumes.",
    ...
    columns: [
      ...
      { key:"college", label:"College / Degree" },
...
function renderStats(){
  const cfg = SECTION_CONFIG.careers;
...
function renderRoleFilter(){
  ...
  const values = [...new Set(DATA.careers.map(d => d.job_title))].filter(Boolean);
...
function renderTable(){
  ...
  tbody.querySelectorAll('.status-select').forEach(sel => {
    sel.addEventListener('change', async function(e){
      ...
      const item = DATA.careers.find(x => x.id === idVal);
      if(item){
  ...
  return `
    <div class="dsection">
      ...
      <div class="dgrid">
        ...
        <div class="dfield"><span class="fl">Job Position Applied</span><span class="fv" style="font-weight:700;">${d.job_title}</span></div>
    ...
    <div class="dsection">
      ...
      <div class="dgrid">
        <div class="dfield"><span class="fl">Currently Pursuing Education</span><span class="fv" style="text-transform:capitalize;">${d.pursuing_education||'No'}</span></div>
...
function openDrawer(id){
  const d = DATA.careers.find(x => x.id === id);
  if(!d) return;

  document.getElementById('dName').textContent = d.name;
  document.getElementById('dSub').textContent = d.email;
  document.getElementById('dBadges').innerHTML = `<span class="badge badge-platinum">Career Applicant</span>`;
  document.getElementById('drawerBody').innerHTML = careerBody(d);
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    if(res.ok) {
      ...
      DATA.careers = allApps.filter(x => x.category === 'career');
    }
  } catch(e){}
  renderStats();
  renderRoleFilter();
